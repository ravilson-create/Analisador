...
export default function AppCliente({ usuario }) {
  ...
  // ── Busca online — ORSE (CEHOP/SE) — só admin: grava direto numa base
  // compartilhada, mesmo tratamento de "subir base" (ver acao "item" de
  // POST /api/bases). ──
  ...
  // ── Análises recentes (já vêm filtradas pelo servidor: públicas +
  // próprias do usuário logado, ou todas se for admin) ──
  ...
  // limite — não há limpeza automática por idade/quantidade. O servidor
  // confere de novo se quem pediu pode excluir (dono ou admin); o botão só
  // aparece já filtrado aqui pra não oferecer uma ação que vai ser negada.
  ...
  return (
    <main style={{ maxWidth: 900, margin: "0 auto", padding: "28px 20px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2, gap: 10, flexWrap: "wrap" }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: C.azul, margin: 0, display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 12, color: "#555" }}>
          <span>{usuario?.nome} · {usuario?.email}{admin ? " · administrador" : ""}</span>
          <button onClick={sair} style={{ ...botaoAcaoEstilo }}>Sair</button>
        </div>
      </div>
      ...
      <Cartao titulo="📚 Bases de referência" subtitulo={admin ? "Importe o SINAPI/ORSE (.xlsx) usado na comparação." : "Bases compartilhadas — você pode ativar/desativar, mas só um administrador importa ou exclui."}>
        {admin && (
        ...
        {carregandoBases ? <p style={{ fontSize: 12, color: "#888" }}>Carregando...</p> : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
            <thead><tr style={{ textAlign: "left", color: "#666" }}>
              <th>Nome</th><th>Competência</th><th>Itens</th><th>Ativa</th>{admin && <th></th>}
            </tr></thead>
            <tbody>
              {bases.map((b) => (
                <tr key={b.id} style={{ borderTop: `1px solid ${C.borda}` }}>
                  ...
                  {admin && <td><button onClick={() => excluirBase(b.id)} style={{ border: "none", background: "none", color: C.vermelho, cursor: "pointer" }}>Excluir</button></td>}
                </tr>
              ))}
              {bases.length === 0 && <tr><td colSpan={admin ? 5 : 4} style={{ padding: "10px 0", color: "#888" }}>Nenhuma base importada ainda.</td></tr>}
            </tbody>
          </table>
        )}
      </Cartao>
      ...
      <Cartao titulo="📋 Análises recentes" subtitulo={admin ? "Você vê as análises de todos os usuários." : "Suas análises + as disparadas automaticamente pelo Tá na Mão."}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
          <thead><tr style={{ textAlign: "left", color: "#666" }}>
            <th>#</th><th>OS</th><th>Arquivo</th><th>Resumo</th><th>De</th><th>Quando</th><th></th><th></th>
          </tr></thead>
          <tbody>
            {analises.map((a) => (
              <tr key={a.id} style={{ borderTop: `1px solid ${C.borda}` }}>
                ...
                <td>{a.usuario_id ? (a.usuario_id === usuario.id ? "você" : (admin ? (a.usuario_nome || a.usuario_email || "outro usuário") : "")) : "automática (Tá na Mão)"}</td>
            ...
            {analises.length === 0 && <tr><td colSpan={8} style={{ padding: "10px 0", color: "#888" }}>Nenhuma análise registrada ainda.</td></tr>}
          </tbody>
        </table>
      </Cartao>
    </main>
