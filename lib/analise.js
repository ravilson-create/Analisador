...
export const parseFlt = (s) => {
  ...
  if (s == null) return 0;
  // Extrai só o trecho numérico (formato BR: milhar com ponto, decimal com
  // vírgula), ignorando prefixos como "R$ " — sem isso, `parseFloat` falha
  // (NaN) em qualquer string que não comece direto por um dígito, e o
  // "|| 0" no fim silenciosamente zera o valor. Foi o que fazia os preços
  // do formato de PDF da Torquato Fernandes (ex.: "R$ 4,27") saírem
  // sempre como 0 na análise, mesmo com o dado correto extraído do PDF.
...
// Uma célula "parece numérica" quando, depois de tirar um eventual prefixo
// de moeda ("R$ "), sobra algo no formato BR (milhar com ponto, decimal com
// vírgula). Usado para localizar quantidade/valorUnit/total varrendo a
// linha do FIM para o começo, em vez de por índice fixo de coluna —
// necessário porque o layout de Analítico da Torquato Fernandes insere, em
// parte das linhas, uma célula em branco extra no meio da linha (por volta
// da coluna de classe), o que empurra unidade/quantidade/valorUnit/total
// uma posição para a direita e quebrava a leitura por índice fixo (metade
// das linhas saía com valorUnit = 0 ou lendo a célula errada). Tanto no
// layout SAGA quanto no Torquato, as últimas 3 colunas de uma linha de
// composição/insumo são sempre, nessa ordem, Quantidade, Valor Unit. e
// Total (= Quantidade × Valor Unit.) — por isso a busca conta 3 células
// numéricas a partir do fim, não 2, antes de considerar a célula anterior
// como Unidade.
const pareceNumero = (v) => {
  if (typeof v === "number") return true;
  if (v == null || v === "") return false;
  return /-?\d{1,3}(?:\.\d{3})*(?:,\d+)?|-?\d+(?:,\d+)?/.test(String(v).replace(/^R\$\s*/i, "").trim());
};
...
export function parseAnalitico(wb) {
  ...
  for (let r = 0; r < rows.length; r++) {
    ...
    // Acha, a partir do fim da linha (ignorando células vazias sobrando),
    // a sequência de células numéricas consecutivas. Tanto no layout SAGA
    // quanto no da Torquato Fernandes essa sequência é sempre, nessa
    // ordem, [Quantidade, Valor Unit., Total(=Quantidade×Valor Unit.)] —
    // ou só [Quantidade, Valor Unit.] quando a coluna Total não existe na
    // página. A célula não-numérica logo antes dessa sequência é a
    // Unidade. Ler por posição relativa ao FIM, em vez de índice fixo,
    // é o que permite lidar com as duas variações de layout observadas
    // (uma célula em branco extra no meio da linha, perto da coluna de
    // classe, que no índice fixo empurrava tudo e fazia metade das linhas
    // do formato Torquato Fernandes sair com valorUnit = 0 ou errado).
    let fim = row.length - 1;
    while (fim > 4 && (row[fim] == null || String(row[fim]).trim() === "")) fim--;
    const numericos = []; // índices, em ordem crescente (esquerda→direita)
    let p = fim;
    while (p > 4 && pareceNumero(row[p]) && numericos.length < 3) { numericos.unshift(p); p--; }
    const idxUnidade = p;
    // Com 3 células numéricas no fim: [Quantidade, Valor Unit., Total].
    // Com 2 (sem coluna Total nessa página): [Quantidade, Valor Unit.].
    // Com 1 (linha incompleta): só dá pra confiar no Valor Unit.
    let idxQtd = null, idxVUnit = null;
    if (numericos.length >= 3) { idxQtd = numericos[numericos.length - 3]; idxVUnit = numericos[numericos.length - 2]; }
    else if (numericos.length === 2) { [idxQtd, idxVUnit] = numericos; }
    else if (numericos.length === 1) { idxVUnit = numericos[0]; }
    const und = String(row[idxUnidade] ?? "").trim();
    const qtd = idxQtd == null ? 0 : (typeof row[idxQtd] === "number" ? row[idxQtd] : parseFlt(row[idxQtd]));
    const vUnit = idxVUnit == null ? 0 : (typeof row[idxVUnit] === "number" ? row[idxVUnit] : parseFlt(row[idxVUnit]));
...
export function analisarItem(item, indiceBases, historico = []) {
  ...
  if (item.proprio) {
    // Antes de desistir de referência, consulta a memória de composições
    // próprias já validadas por algum fiscal em análises anteriores
    // (dados_compartilhados.composicoes, gravada em
    // /api/analise-automatica/corrigir na ação "composicao"). Só a memória
    // é consultada aqui (nunca SINAPI/ORSE) — um item "Próprio" continua
    // sem código de tabela pública; o que muda é que, se o MESMO serviço já
    // foi validado antes, isso agora vira parâmetro de comparação.
    const entradasMemoria = indiceBases.entradas.filter((e) => e.s?.tabela === "Memória (fiscal)");
    const idxMemoria = {
      entradas: entradasMemoria,
      porCodigo: new Map(entradasMemoria.filter((e) => e.cod).map((e) => [e.cod, e.s])),
      temSinapi: false, temOrse: false,
    };
    const refMem = refPorDescricaoIndice(item.codigo, item.descricao, idxMemoria);

    if (refMem) {
      const fatorReajM = item.reajuste || 1;
      const fatorDesM = 1 - (item.desagio || 0);
      const fatorBdiM = 1 + (item.bdi || BDI_PADRAO);
      origemRef = refMem.tabela || "Memória (fiscal)";
      const diff = item.preco - refMem.preco;
      if (Math.abs(diff) > 0.01) {
        const pct = (diff / refMem.preco) * 100;
        alertas.push(
          `COMPOSIÇÃO PRÓPRIA DIVERGE DE VALIDAÇÃO ANTERIOR: preço calculado R$ ${fmt(item.preco)} vs. ${fmt(refMem.preco)} já validado antes pelo fiscal para o mesmo serviço ` +
          `(${pct > 0 ? "+" : ""}${pct.toFixed(2)}%).`
        );
        status = pct > 0 ? "nao_conforme" : "atenção";
      } else {
        infos.push(`Composição própria confere com validação anterior do fiscal para o mesmo serviço (${origemRef}).`);
        if (status !== "nao_conforme") status = "conforme";
      }
      if (!item.quantidade || item.quantidade <= 0) { alertas.push("Quantidade zerada ou ausente."); status = "nao_conforme"; }

      const excedentePrecoMem = item.preco > refMem.preco
        ? (item.preco - refMem.preco) * fatorReajM * fatorDesM * fatorBdiM * (item.quantidade || 0) : 0;
      const excedenteItemMem = excedentePrecoMem + excedenteQuantidade;

      return {
        ...item, ref: refMem, origemRef, alertas, alertasMem, infos, status,
        ocorrencias: [], tendencia: null, precoComAjuste, precoComBdi, totalItem,
        excedenteItem: excedenteItemMem, excedentePreco: excedentePrecoMem, excedenteQuantidade
      };
    }
